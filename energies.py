...
def cvx_hull_ls(X, W, chunk_size=CHUNK_SIZE, returnH=True):
    n, d = X.shape
    k = W.shape[0]
    if returnH:
        H = np.zeros((n,k))
    e = np.ones(k)
    X_ = cp.Parameter((chunk_size,d), nonneg=True)
    W_ = W[:]
    H_ = cp.Variable((chunk_size, k), nonneg=True)

    ix = 0
    energy_val = 0.0
    energy_values = np.zeros(n)
    obj = cp.Minimize(cp.sum_squares(X_ - H_ @ W_))
    constraint = [H_ @ e <= 1]
    prob = cp.Problem(obj, constraint)
    while ix + chunk_size < n:
        X_.value = X[ix:ix+chunk_size,:]
        energy_val += prob.solve(ignore_dpp=True)
        energy_values[ix:ix+chunk_size] = np.linalg.norm(X_.value - H_.value @ W_, axis=1)**2.
        if returnH:
            H[ix:ix+chunk_size, :] = H_.value
        ix += chunk_size

    H_ = cp.Variable((n - ix, k), nonneg=True)
    X_ = cp.Parameter((n-ix,d), nonneg=True)
    X_.value = X[ix:,:]
    obj = cp.Minimize(cp.sum_squares(X_ - H_ @ W_))
    constraint = [H_ @ e <= 1]
    prob = cp.Problem(obj, constraint)
    energy_val += prob.solve(ignore_dpp=True)
    energy_values[ix:] = np.linalg.norm(X_.value - H_.value @ W_, axis=1)**2.
    if returnH:
        H[ix:,:] = H_.value
        return H, energy_val, energy_values
    return None, energy_val, energy_values
"""
...
class ConvexHullEnergy_nnls(Energy):
    def __init__(self, X, k, n_jobs=4):
        super().__init__(X, k)
        self.sparse_flag = sps.issparse(self.X)
        self.W = np.zeros((self.k, self.d))
    ...
    def add(self, i):
        ...
        self.H[self.unselected_inds,:self.k_sel] = np.vstack([out[0] for out in outs])
        self.dists[self.indices] = 0.0
        self.dists[self.unselected_inds] = np.array([out[1]**2. for out in outs])
        self.energy = self.dists.sum()
        self.energy_values.append(self.energy)
        return
    ...
    def init_set(self, inds): # done so that we can track the energy values throughout all choices.
        ...
        # self.unselected_inds = np.delete(np.arange(self.n), self.indices)
        # outs = Parallel(n_jobs=self.n_jobs)(
        #             delayed(nnls)(self.W.T, self.X[j,:]) for j in self.unselected_inds
        #         )
        # self.H[self.indices, :self.k_sel] = np.eye(self.k_sel)
        # self.H[self.unselected_inds, :self.k_sel] = np.vstack([out[0] for out in outs])
        # self.dists[self.indices] = 0.0
    ...
    def look_ahead(self, candidates=None):
        ...
        for c in candidates:
            outs = Parallel(n_jobs=self.n_jobs)(
                    delayed(nnls)(np.vstack((self.W[:self.k_sel,:], self.X[c,:].reshape(1,-1))).T, self.X[j,:]) for j in self.unselected_inds if j != c 
                )
            dists_c = np.zeros(self.n)
            dists_c[np.setdiff1d(self.unselected_inds, [c])] = np.array([out[1]**2. for out in outs])
            cand_la_results[c]['dists'] = dists_c
            cand_la_results[c]['energy'] = cand_la_results[c]['dists'].sum()
